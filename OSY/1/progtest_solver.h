...
//-------------------------------------------------------------------------------------------------
/**
 * The function combines fragments of a single message and tries to recover the
 * message payload:
 *  - the function checks that all fragments belong to the same message
 *  - for each permutation, the payload is extracted, serialized and checked for
 * valid CRC
 *  - if the CRC matches, then the message payload (CRC + bitfield) is passes to
 * the callback
 *
 * @param [in] fragments   the fragments to process
 * @param [in] cnt         the # of fragments
 * @param      recvFn      a callback to call for each permutation where the CRC
 * matches. The callback is given the payload (1-st argument: const uint8_t *)
 * and the length of the payload in bits (2-nd argument: size_t). The callback
 * is given the entire message payload, i.e., CRC32 followed by the bitfield.
 * The length parameter is set to the length of the entire payload, i.e., 32
 * must be subtracted to get the # of bits in the bitfield.
 * @return the number of different valid payloads extracted (i.e., the total #
 * times the callback was actually called). 0 is returned if the input fragments
 * do not form a valid  message (e.g. some fragments are missing) or if the
 * input fragments mix two or more messages with different IDs.
 * @note the function is rather slow - the time complexity is n! (check all
 * permutations).
 ...
 * bonus test.
 */
uint32_t FindPermutations(const uint64_t *fragments, size_t cnt,
